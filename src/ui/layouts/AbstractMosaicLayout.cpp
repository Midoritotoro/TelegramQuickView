...
namespace Ui::Layout {
	...
	AbstractMosaicLayout::AbstractMosaicLayout(int bigWidth) :
		_bigWidth(bigWidth)
		, _minWidth(style::minimumMessageWidth)
		, _minHeight(style::minimumMessageHeight)
	{}
	...
	QRect AbstractMosaicLayout::findRect(int index) const {
		...
		for (auto row = 0; row != rows; ++row) {
			...
			for (const auto& item : inlineRow.items) {
				...
				if ((left + w) > fromX && item->position() == index)
					return QRect(
						left + _padding.left(),
						top + _padding.top(),
						item->width(),
	...
	not_null<AbstractLayoutItem*> AbstractMosaicLayout::itemAt(int index) const {
		const auto& [row, column] = indexToPosition(index);
